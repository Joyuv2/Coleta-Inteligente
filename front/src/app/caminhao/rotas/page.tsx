...
export default function Page() {
  ...
  return(
    <div className="h-screen flex flex-col">
      <Navbar drops={drops} links={links} location={["Início","Caminhões","Rotas"]} />
      <div className="p-6 w-full h-full flex flex-row gap-6">
        <div className="w-1/2 h-full bg-background2 p-3">
          <MapPoints position={[-6.455410, -37.094683]} zoom={15}/>
        </div>
        <div className="w-1/2 h-full flex flex-row flex-wrap bg-background2 p-3 gap-5">
          {routes.map((el, ind) => (
